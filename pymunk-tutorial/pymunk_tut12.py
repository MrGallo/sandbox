...
def collision_begin(arbiter, space, data):
    print(dir(arbiter))
    for attr in [attr for attr in dir(arbiter) if not attr.startswith("_")]:
        print(f"{attr}: {getattr(arbiter, attr)}")
    return True
...
def collision_post_solve(arbiter, space, data):
    print(f"{arbiter.total_ke = }")
    print(f"{arbiter.total_impulse = }")
